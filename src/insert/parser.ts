      ...
      : First extends "DEFAULT"
        ? NextToken<Rest> extends ["VALUES", infer AfterValues extends string]
          ? BuildInsertClause<
              Table,
              Columns,
              InsertValuesClause<[InsertValueRow<[{ type: "Default" }]>]>,
              AfterValues
            >
 ...
 */
type ParseConflictBody<T extends string> = ParseConflictTarget<T> extends infer TargetResult
  ? TargetResult extends {
      target: infer Target
      rest: infer AfterTarget extends string
    }
    ? ParseConflictAction<AfterTarget> extends infer ActionResult
      ? ActionResult extends {
...
type ParseConflictTarget<T extends string> = NextToken<T> extends [
  ...
  : NextToken<T> extends ["ON", infer Rest extends string]
    ? NextToken<Rest> extends ["CONSTRAINT", infer AfterConstraint extends string]
      ? NextToken<AfterConstraint> extends [
 ...
 */
type ParseUpdateSets<T extends string, Acc extends ConflictUpdateSet[] = []> = ExtractUntil<
  T,
  "RETURNING" | "WHERE"
> extends [infer SetsPart extends string, infer Rest extends string]
 ...
 */
type ParseSetValue<T extends string> = T extends `EXCLUDED . ${infer Col}`
  ? "EXCLUDED"
 ...
 */
type ParseTableRef<T extends string> = Trim<T> extends `${infer Schema}.${infer Table}`
