...
// ============================================================================
// Exported Parser Utilities (for builder and other consumers)
// ============================================================================

/** Parse a table reference string into TableRef AST */
export type { ParseTableRef };

/** Parse a single column expression into SelectItem AST */
export type { ParseSingleColumn };

/** Parse a list of column expressions into SelectItem[] */
export type { ParseColumnList };

/** Parse a column reference type (table.column, column, etc.) */
export type { ParseColumnRefType };

/** Extract column name for default alias */
export type { ExtractColumnName };

/** Check if expression is complex (contains functions, operators, etc.) */
export type { IsComplexExpression };
...
export type { ParseSingleJoin };

/** Check if string is a simple identifier */
export type { IsSimpleIdentifier };
 ...
 */
type ParseSelectQuery<T extends string> = ParseSelectQueryWithRest<T> extends
    infer Result ? Result extends {
        query: infer Q extends SelectClause;
        rest: infer Rest extends string;
    } ? CheckForUnion<Q, Rest>
 ...
 */
type ParseWithAndSelect<T extends string> =
    ParseWithAndSelectWithRest<T> extends infer Result ? Result extends {
            query: infer Q extends SelectClause;
            rest: infer Rest extends string;
        } ? CheckForUnion<Q, Rest>
        : Result
 ...
 */
type ExtractCTEQuery<T extends string> =
 ...
 */
type IsAggregate<T extends string> = T extends `COUNT ${string}` ? true
...
type ParseSimpleColumn<T extends string> =
    ...
    IsLiteralExpression<T> extends true ? ParseLiteralColumn<T>
        ...
        : IsIntervalExpression<T> extends true ? ParseIntervalColumn<T>
        // Check for table.* wildcard
        : IsTableWildcard<T> extends true ? ParseTableWildcard<T>
        // Check for EXISTS/NOT EXISTS (must be before scalar subquery)
        : IsExistsExpression<T> extends true ? ParseExistsColumn<T>
        // Check for scalar subquery (parenthesized SELECT)
        : IsSubqueryExpression<T> extends true ? ParseSubqueryColumn<T>
        // Check for CAST function (must be before general complex expression check)
        : IsCastExpression<T> extends true ? ParseCastColumn<T>
        // Check for complex expression (contains JSON operators or parentheses with operators)
        : IsComplexExpression<T> extends true ? ParseComplexColumn<T>
 ...
 */
type ExtractAliasFromRemainder<T extends string> = Trim<T> extends
    `AS ${infer AliasRest}`
 ...
 * e.g., ")::text" -> "text"
 */
type ExtractSubqueryCastType<T extends string> = Trim<T> extends
 ...
 */
type ExtractParenthesizedContent<T extends string> = Trim<T> extends
 ...
 */
type IsSimpleIdentifier<T extends string> = T extends "" ? false
 ...
 */
type ParseDerivedTableAlias<T extends string> = NextToken<T> extends
 ...
 */
type ParseTableRef<T extends string> = Trim<T> extends
 ...
 */
type ParseOptionalClauses<T extends string> =
 ...
 */
type ParseOptionalClausesWithRest<T extends string> = ParseJoins<T> extends
    infer JoinResult
    ? JoinResult extends
 ...
 * Parse a single JOIN clause
 */
type ParseSingleJoin<T extends string> = [ ExtractJoinType<T> ] extends
